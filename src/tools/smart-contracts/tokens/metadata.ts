 ...
 */
export async function getMetadata(contract: ContractAbstraction): Promise<TZip17Metadata | undefined> {
  ...
  if (MichelsonMap.isMichelsonMap(faMetadata)) {
    ...
    const unwrapped: TZip17Metadata = await unwrapMichelsonMap<TZip17Metadata>(faMetadata, contract.address);
    return unwrapped;
  }
}
