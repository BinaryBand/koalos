...
describe('FA-1.2 token contract', () => {
  ...
  it('fetch total token supply', async () => {
    const supply: BigNumber = await (await fa12Instance).getTotalSupply();
    expect(supply.toNumber()).toBe(1699968);
  });

  it('fetch token balance', async () => {
    const balance: BigNumber = await (await fa12Instance).getBalance(burnAddress);
    expect(balance.toNumber()).toBe(15776);
  });

  it('create token transfer params', async () => {
    const transferParams: TransactionOperationParameter = await (
      await fa12Instance
    ).transfer(revealedAddress, burnAddress, 1000);
  ...
  it('get contract metadata from Tezos local URI, tezos-storage:data', async () => {
    const metadata: TZip17Metadata | undefined = await (await fa12Instance).getMetadata();
  ...
  it('get token metadata', async () => {
    const tokenMetadata: TZip21TokenMetadata | undefined = await (await fa12Instance).getTokenMetadata();
...
//   const fa2Instance_1: Fa2Token = new Fa2Token(fa2Contract_1, blockchainInstance);
//   const fa2Instance_2: Fa2Token = new Fa2Token(fa2Contract_2, blockchainInstance);
