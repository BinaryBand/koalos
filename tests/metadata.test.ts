...
describe('token metadata', () => {
  ...
  it('fetch token metadata from assets.token_metadata[0]["token_info"]', async () => {
    ...
    const crunchTokenMetadata: TZip21TokenMetadata | undefined = await getTokenMetadata(crunch);
  ...
  it('QUIPU token metadata', async () => {
    ...
    const quipuTokenMetadata: TZip21TokenMetadata | undefined = await getTokenMetadata(quipu, 0);
